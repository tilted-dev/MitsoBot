...
type TodayScheduleButtonPayloadHandler(tgClient:ITelegramBotClient, mitsoService:IMitsoService) =
    interface IGenericButtonPayloadHandler<TodayScheduleButtonPayloadHandler> with
        member this.ProcessPayload requestData = async {
            ...
            | Ok schedule ->
                do! tgClient.SendTextMessageAsync(requestData.User.Id, text=schedule.PrettifyData())
                    |> Async.AwaitTask
                    |> Async.Ignore
